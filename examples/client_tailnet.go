...
func main() {
	...
	if err != nil {
		...
		fmt.Println("\nNote: This requires both client and receiver to have Tailscale tsnet configured.")
		fmt.Println("Make sure the receiver is running with Tailscale integration enabled.")
	} else {
...
func printTailscaleInstructions() {
	...
	fmt.Println("   - Uncomment tsnet code in receiver_tailnet.go")
	fmt.Println("   - Add 'tailscale.com v1.76.1' to go.mod")
	fmt.Println()
	fmt.Println("4. Start the receiver:")
	fmt.Println("   go run receiver_tailnet.go")
	fmt.Println()
